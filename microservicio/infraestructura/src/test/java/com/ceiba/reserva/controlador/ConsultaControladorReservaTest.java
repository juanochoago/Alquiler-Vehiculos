...
class ConsultaControladorReservaTest {
    ...
    void deberiaListarReserva() throws Exception {
        // arrange
        Long id = 1L;
        // act - assert
        mocMvc.perform(get("/vehiculo/{id}", id)
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(jsonPath("idCliente", is(1010101011)))
                .andExpect(jsonPath("nombreCliente", is("Juan")))
                .andExpect(jsonPath("tipoVehiculo", is(1)))
                .andExpect(jsonPath("numeroDias", is(3)))
                .andExpect(jsonPath("valor", is(300000)));
